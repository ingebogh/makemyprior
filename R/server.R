...
# source("functions_for_server_shiny.R")
# source("../functions_both.R")
# source("../functions_calculate_pc_prior.R")
...
server <- function(input, output, session) {
  ...
  observe({
    if (is.null(pd$w)) pd$w <- .initial_args$.initial_prior_w
    if (is.null(pd$V)) pd$V <- .initial_args$.initial_prior_V
    if (is.null(pd$var)) pd$var <- .initial_args$.initial_prior_var
  })

  # initializing the tree based on user input (node_data)
  nd <- reactiveValues(x = NULL)
  observe({
    if (is.null(nd$x)) nd$x <- initialize_nodes(.initial_args)
  })

  td <- reactiveValues(x = NULL) # for message to user
  node_out <- reactiveValues(x = NULL)
  gd <- reactiveValues(x = initialize_guide(.initial_args$.guide)) # keeping track of the guide
  ...
  observeEvent(input$current_node_id, {
    ...
    model_data_tmp <- .initial_args$.model_data
  ...
  observeEvent(length(input$weight_prior) & input$median & is.null(input$basemodel), {
  ...
  #################################################################### what used to be in a separate script
  ...
  observe({
    ...
    if (gd$x$guide_is_running && gd$x$which_totvar > 0){
      tmp <- totvar_part_guide(nd$x, pd$V, gd$x)
      gd$x <- tmp$guide_data
    }
  ...
  output$all_plots_guide <- renderPlot({
    if (gd$x$guide_is_running) plot_priors(pd$w, pd$V, pd$var, calculate_pc_prior(nd$x, pd$w, .initial_args$.indata$covmats), nd$x)
  },
  height = function() 150*ceiling((length(.initial_args$.nodenames))/3),
  bg = "transparent")
  ...
  output$basemodel_name <- renderText({
    ...
    if (is_split_node(nd$x, input)){
      ...
      if (prior_info$prior != "pc") {
        ...
        basemod_id <- prior_info$param$basemodel_node
  ...
  observe({
    if (!isValid_median()){
      updateNumericInput(session, "median", value = 0.25)
    }
  })
  ...
  output$graph_guide <- renderVisNetwork({
  ...
  plot_priors_pushed <- eventReactive(input$make_plot, {
    plot_priors(pd$w, pd$V, pd$var, calculate_pc_prior(nd$x, pd$w, .initial_args$.indata$covmats), nd$x, input$current_node_id)
  })
  ...
  output$param_eq <- renderUI({
    if (TRUE){
      withMathJax(helpText(
        # make_var_eq_latex(nd$x, pd$w, pd$V, pd$var, .initial_args$.response_name)
        text_latex_param(nd$x, pd$w, pd$V, pd$var, .initial_args$.response_name, input$param_type_eq)
      ))
    }
  })
  ...
  output$prior_distributions <- renderUI({
    #if ("variance_equations" %in% input$sidebarpanel){
    if (TRUE){
      withMathJax(helpText(
  ...
  output$model_eq <- renderText({.initial_args$.model_eq})
  ...
  observeEvent(input$reset, {
    ...
    reset("var_prior")
    # reset objects
    pd$w <- .initial_args$.initial_prior_w
    pd$V <- .initial_args$.initial_prior_V
    pd$var <- .initial_args$.initial_prior_var
    nd$x <- initialize_nodes(.initial_args)
    gd$x <- initialize_guide(F)
  })
