...
* $\beta$ is a coefficient with a $\mathcal{N}(0, 1000^2)$ prior for $x_{i,j}$, which is an indicator classifying cluster $j$ in county $i$ as urban ($x_{i,j} = 1$) or rural ($x_{i,j} = 0$),
* $\nu_i \sim \mathcal{N}_n(0, \sigma_{\nu}^2)$ is an i.i.d. random effect for cluster
* $v_i \sim \mathcal{N}_n(0, \sigma_v^2)$ is an i.i.d. random effect for county, and
